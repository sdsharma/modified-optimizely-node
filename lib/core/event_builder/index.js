...
function getCommonParams(configObj, userId, attributes) {
  ...
  params[urlParams.endUserId] = sprintf('oeu-%s', userId);

  if (attributes) {
    var attributeParams = getAttributeParams(configObj, attributes);
    _.extend(params, attributeParams);
  }
 ...
 * @param  {string} eventKey     Goal key representing the event which needs to be recorded
 * @param  {string} userId       ID for user
 * @param  {Object} params       Object representing event params for impression/conversion event
 ...
 */
function getExperimentVariationParams(configObj, eventKey, userId, variationIds) {
  ...
  var experimentKeys = _.filter(Object.keys(configObj.experimentKeyMap), function(experimentKey) {
    ...
    if (experimentId && (experimentIdsForGoal.indexOf(experimentId) !== -1)) {
...
module.exports = {
  ...
  createImpressionEventParams: function(configObj, experimentKey, variationId, userId, attributes) {
    ...
    return _.extend(params, impressionGoalParams, experimentParams);
  },
   ...
   */
  createConversionEventParams: function(configObj, eventKey, userId, attributes, eventValue, variationIds) {
    var params = getCommonParams(configObj, userId, attributes);
    var conversionGoalParams = getConversionGoalParams(configObj, eventKey, eventValue);
    var experimentVariationParams = getExperimentVariationParams(configObj, eventKey, userId, variationIds);
    return _.extend(params, conversionGoalParams, experimentVariationParams);
  },
};
